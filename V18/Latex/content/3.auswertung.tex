...
The measurement is performed over a duration of about 120 minutes, 
...
\end{align*}
The fit is displayed in \autoref{fig:eu_calibration_fit}.
\begin{figure}[h!]
	\centering
	\includegraphics[width=0.7\textwidth]{../build/Eu-Energie_linear.pdf}
	\caption{Linear fit of the energy calibration. The identified peaks are shown as blue dots, while the fitted line is shown in red.}
	\label{fig:eu_calibration_fit}
\end{figure}
...
\subsection{Gamma spectrum of Cesium}
A $^{137}\text{Cs}$ probe was measured over a time period of about $t=\SI{14400}{\second}$.
...
\begin{equation}
	g(x) = s \cdot \frac{1}{\sqrt{2 \pi} \sigma} \exp{\left( -\frac{1}{2} \frac{(x - \mu)^{2}}{\sigma^2} \right)} + b.
	\label{eqn:fitplus}
\end{equation}
...
\begin{table}[ht]
	\centering
	\caption{Fit parameters of the photo peak of the $^{137}\text{Cs}$ spectrum with more than $\SI{150}{\kilo\electronvolt}$.}
	...
	\end{tabular}
\end{table}
The full width at half maximum (FWHM) and full width at tenth maximum (FWTM) of the photo peak of the $^{137}\text{Cs}$ spectrum are estimated. 
...
\begin{equation}
	\frac{\text{FWHM}}{\text{FWTM}} = 0.55 \pm 0.0004.
\end{equation}
...
\begin{align}
	\alpha & = \num{0.008 \pm 0.000004}    \\
	\beta  & = \num{1.0 \pm 0.8}.
\end{align}
This yield of the compton continuum as estimated by this fit amounts to
\begin{equation}
	N = \num{15498.84830 \pm 0.000002}.
\end{equation}

In the final step of this analysis, 
the content of the Compton continuum
and the full-energy peak is compared with the absorption probabilities of the Compton and photoelectric effects. 
For this purpose, 
the formula
\begin{equation}
...
\end{equation}
is used, 
where $l = \SI{3.9}{\centi\meter}$ is the detector length and $\mu$ is the corresponding absorption coefficient. 
The resulting values are
\begin{align}
    \mu_\text{Compton} &= \SI{0.38(3)}{\per\centi\meter}, &
    p_\text{Compton} &= \SI{74(7)}{\percent}, \notag\\
    \mu_\text{Photo} &= \SI{0.002(3)}{\per\centi\meter}, &
    p_\text{Photo} &= \SI{2.7(1)}{\percent}.
\end{align}
Next, the entries of the individual energies up to the Compton edge are summed.
For the full-energy peak, 
a window of \SI{21}{\kilo\electronvolt} (from minimumto minimum) is used before and after the maximum to determine the peak content.
From this, 
the counts are determined as
\begin{align}
    Z_\text{Compton} &= \num{9.0(8)e4}, &
    Z_\text{Full} &= \num{3.39(7)e4}.
\end{align}
...
\begin{table}
	...
	\midrule
		 7.13 &  0.163 &  0.007 &    274.97 &  0.01 &  909 &  19 \\
...
The resulting activities are given in Table~\ref{tab:urresults}.
\begin{table}[h!]
	\centering
